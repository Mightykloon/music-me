...
export class SpotifyProvider implements MusicProvider {
  ...
  async getPlaylistTracks(
    ...
    let url: string | null = `/playlists/${playlistId}/tracks?limit=100&market=US&additional_types=track`;
  ...
  async getPlaylistTracksPage(
    ...
    const data = await this.fetchApi(
      accessToken,
      `/playlists/${playlistId}/tracks?offset=${offset}&limit=${limit}&market=US&additional_types=track`
    );
